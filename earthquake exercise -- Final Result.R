...
ca1<-sqldf("select * from eq1 where (place like ('%California') or place like ('%Nevada') or place like '%Oregon') and type='earthquake' and mag>=0.5 order by mag desc")
ca2<-sqldf("select * from eq2 where (place like ('%California') or place like ('%Nevada') or place like '%Oregon') and type='earthquake' and mag>=0.5 order by mag desc")
ca3<-sqldf("select * from eq3 where (place like ('%California') or place like ('%Nevada') or place like '%Oregon') and type='earthquake' and mag>=0.5 order by mag desc")
...
map(database="state",region=c("california","nevada","oregon"),xlim=c(-130,-110),fill=TRUE,col="#FFCC99")
...
title("Map of Recent Earthquakes in CA, NV, and OR")
map.axes()
